...
import {
  ...
  FaGraduationCap, FaSchool, FaUserCircle, FaBookReader,
...
export default function AdminUsers() {
  ...
  // Form state mein qualification ko common rakho
...
const fetchUsers = async () => {
  try {
    ...
    const res = await API.get(`/admin/users/advanced?${params}`);
    ...
    // IF Addvandesd Route failed then use the fallback rout
    if (error.response?.status === 404) {
      console.log("Advanced route not found, trying fallback...");
      await fetchUsersFallback();
    } else {
      errorAlert("Error", "Failed to load users");
    }
  } finally {
    setLoading(false);
  }
...
// Fallback function - If Advanced Route do not got
const fetchUsersFallback = async () => {
  ...
  const getRoleIcon = (role) => {
    switch (role) {
      ...
      case 'librarian': return <FaBookReader className="text-indigo-600" />;
  ...
  return (
    <div className={`space-y-4 md:space-y-6 px-2 sm:px-4 ${isMobile ? 'pt-16 pb-24' : ''}`}>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u._id} className="border-t hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-3">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${u.isActive
          ...
          {users.map((u) => (
            <div key={u._id} className="p-4 hover:bg-gray-50 transition-colors">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${u.isActive
                  ? 'bg-green-100 text-green-600'
                  : 'bg-red-100 text-red-600'
                  }`}>
                  {u.isActive ? 'Active' : 'Inactive'}
              ...
              <div className="flex gap-2 pt-2 border-t">
                <button
                  onClick={() => viewUserDetails(u._id)}
                  className="flex-1 flex items-center justify-center gap-2 bg-green-100 text-green-600 px-3 py-2 rounded-lg text-sm hover:bg-green-200"
                >
                  <FaEye /> View
                </button>
                <button
                  onClick={() => editUser(u)}
                  className="flex-1 flex items-center justify-center gap-2 bg-blue-100 text-blue-600 px-3 py-2 rounded-lg text-sm hover:bg-blue-200"
                >
                  <FaEdit /> Edit
                </button>
                <button
                  onClick={() => toggleStatus(u._id, u.isActive)}
                  className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm ${u.isActive
                ...
                <button
                  onClick={() => changePassword(u._id)}
                  className="flex-1 flex items-center justify-center gap-2 bg-red-100 text-red-600 px-3 py-2 rounded-lg text-sm hover:bg-red-200"
                >
                  <FaKey /> Password
                </button>
              </div>
      ...
      {pagination.totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-lg">
            ...
            <div className="p-6 space-y-6">
              {viewUserLoading ? (
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg">
                      ...
                      <div>
                        ...
                        <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${viewUser.user.isActive
                          ? 'bg-green-100 text-green-600'
                          : 'bg-red-100 text-red-600'
                          }`}>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-blue-50 p-4 rounded-lg">
                        ...
                        <div>
                          ...
                          <p className="font-medium">{viewUser.user.qualification || 'Not specified'}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500">Joining Date</p>
                          <p className="font-medium">{viewUser.user.joiningDate || 'Not specified'}</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {/* FIX - Add Accountant section in View Modal */}
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-orange-50 p-4 rounded-lg">
                        <div>
                          ...
                          <p className="font-medium">{viewUser.user.employeeId || 'Not assigned'}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500">Qualification</p>
                          <p className="font-medium">{viewUser.user.qualification || 'Not specified'}</p>
                        ...
                        <div>
                          <p className="text-xs text-gray-500">Joining Date</p>
                          <p className="font-medium">{viewUser.user.joiningDate ? formatDate(viewUser.user.joiningDate) : 'Not specified'}</p>
                        </div>
                      </div>
                    </div>
                  )}
