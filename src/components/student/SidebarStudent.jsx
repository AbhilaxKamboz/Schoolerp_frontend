import { FaHome, FaUserGraduate, FaCalendarCheck, FaTasks, FaChartBar, FaChalkboard, FaSignOutAlt, FaBell, FaCog, FaQuestionCircle, FaChevronLeft, FaChevronRight, FaTimes, FaGraduationCap,FaBook } from "react-icons/fa";
import { useState, useEffect } from "react";
import { useAuth } from "../../context/AuthContext";
import { useNavigate } from "react-router-dom";
import Swal from "sweetalert2";

export default function StudentSidebar({ active, setActive, isMobile, sidebarOpen, onClose }) {
  ...
  const menuItems = [
    ...
    { id: "subjects", label: "My Subjects", icon: <FaChalkboard />, color: "purple" },
    { id: "attendance", label: "Attendance", icon: <FaCalendarCheck />, color: "orange" },
    { id: "assignments", label: "Assignments", icon: <FaTasks />, color: "pink" },
    { id: "marks", label: "My Marks", icon: <FaChartBar />, color: "indigo" },
    { id: "library", label: "Library", icon: <FaBook />, color: "teal" },
    { id: "profile", label: "My Profile", icon: <FaUserGraduate />, color: "green" },
  ];
  ...
  const getColorClasses = (color, isActive) => {
    const colors = {
      ...
      cyan: {
        active: "bg-cyan-50 text-cyan-600 border-cyan-600",
      ...
      >
        ...
        `}>
          <div className={`flex ${isCollapsed && !isHovered ? 'flex-col' : 'items-center'} gap-3`}>
            <div className="w-10 h-10 bg-gradient-to-r from-green-500 to-green-600 rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
              S
            </div>
            {showLabels && (
              <div className="flex-1 min-w-0">
                <p className="font-medium text-sm truncate">Student Name</p>
                <p className="text-xs text-gray-500 truncate">Class 10-A | Roll: 25</p>
              </div>
            )}
          </div>
        </div>
